...
SleepSpecial::SleepSpecial(int _rate) : rate(_rate) , chrono_prev(std::chrono::high_resolution_clock::now())
{
}

void SleepSpecial::sleep_rate(int& iterations) {
        auto chrono_now = std::chrono::high_resolution_clock::now();
        auto delta_chrono = std::chrono::milliseconds(rate) - std::chrono::duration_cast<std::chrono::milliseconds>(chrono_now - chrono_prev);
