...
int main() try {
    ...
    unsigned int prev_ticks = SDL_GetTicks(); // an unsigned 32-bit value representing the number of milliseconds since the SDL library initialized.
    ...
    while (true) {
        ...
        if (is_running) {
            if (is_moving_left) {
                position -= frame_delta * 0.2; // move left si aumento el porcentaje, aumenta la velocidad
            } else {
                position += frame_delta * 0.2; // move right
            }
            run_phase = (frame_ticks / 100) % 6; // animation phase. frame_ticks es la posicion actual dividido la cantidad de imagenes
        } else {
            run_phase = 0;
        }
        ...
        renderer.Copy(
            background,
            Rect(0, 0, renderer.GetOutputWidth(), renderer.GetOutputHeight()) // full window background
        );

        int tile_size = 50; // Size of the tile in pixels after scaling
        int num_tiles_x = renderer.GetOutputWidth() / tile_size + 1; // Number of tiles needed horizontally
        ...
        renderer.Copy(
            sprites,
            Rect(src_x, src_y, 32, 32), // (x, y, w, h): (x, y) toman una coordenada de la imagen (w, h) le indicamos el alto y ancho
            Rect((int)position, vcenter - 50, 50, 50), // le indicamos en que parte de la pantalla aparece, y el tamaño
            0.0, // no rotation
