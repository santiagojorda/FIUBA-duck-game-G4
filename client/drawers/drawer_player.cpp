...
void DrawerPlayer::draw(SDL2pp::Renderer& renderer) {
    // entiendo que el frame me dice la parte de la animacion... ??????????? si no es así, yo tengo que controlarlo
    int src_x = DUCK_INITIAL_X + SIZE_DUCK_SPRITE * player.frame; //   int src_x = DUCK_INITIAL_X + SIZE_DUCK_SPRITE * run_phase;
 ...
 void DrawerPlayer::update_player(const player_t& updated_player) {
